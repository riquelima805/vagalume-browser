...
class StorageClient(private val registry: GossipRegistry) {
    ...
    // Para conteúdo de site: a fileKey já vem pronta no manifesto (publicada de
    // propósito, ver GossipRegistry.SiteRoute), não existe masterKey privado pra
    // derivar dela — é o mesmo downloadBlock de sempre, só sem o passo de derivação.
    fun downloadFileWithKey(fileId: String, fileKey: ByteArray): ByteArray {
        val file = registry.getFile(fileId) ?: error("arquivo não encontrado nos metadados conhecidos (ainda não chegou via gossip?)")
        val out = java.io.ByteArrayOutputStream(file.originalLength)
        for (block in file.blocks.sortedBy { it.blockIndex }) {
            out.write(downloadBlock(file, block, fileKey))
        }
